/**
 * @file gen-ui/action-step.tsx
 * @description
 * AI 에이전트가 실행하는 다양한 도구(tool) 액션을 표시하는 핵심 컴포넌트.
 * Shell, Patch, Grep, TextEditor, MCP 등 10가지 이상의 액션 타입을 지원하며,
 * 각 액션의 실행 상태와 결과를 시각화합니다.
 *
 * **지원하는 액션 타입:**
 * - shell: Shell 명령 실행
 * - apply-patch: Diff 패치 적용
 * - install_dependencies: 의존성 설치
 * - scratchpad: 메모 저장
 * - get_url_content: URL 콘텐츠 가져오기
 * - grep: 파일 검색
...
/**
 * BaseActionProps - 모든 액션의 공통 속성
 * @interface
 * @property {string} status - 액션 상태 ("loading" | "generating" | "done")
 * @property {boolean} [success] - 성공 여부 (done 상태에서)
 * @property {string} [reasoningText] - AI reasoning 텍스트
 * @property {string} [summaryText] - 요약 텍스트
 */
...
/**
 * Shell 명령 실행 Props
 * @note Partial<ShellToolArgs>로 감싼 이유: 스트리밍 중 undefined 가능
 */
 ...
 * - `shouldShowToggle()`: 토글 버튼 표시 여부
 * - `renderHeaderIcon()`: 타입별 헤더 아이콘
 * - `renderHeaderContent()`: 타입별 헤더 콘텐츠
 * - `renderContent()`: 타입별 본문 콘텐츠
 */
