/**
 * @file prompts.ts
 * @description 이 파일은 Open SWE V2 에이전트의 행동과 정체성을 정의하는 마스터 시스템 프롬프트를
 * 생성하는 역할을 합니다. `getCodingInstructions` 함수는 에이전트의 핵심 지침, 행동 규칙,
 * 어조 및 스타일, 도구와 하위 에이전트의 올바른 사용법, 그리고 따라야 할 제약 조건 등을
 * 상세하게 기술한 거대한 템플릿 문자열을 반환합니다. 이 프롬프트는 에이전트가 모든 상호작용에서
 * 일관되고, 안전하며, 효과적으로 작동하도록 보장하는 가장 중요한 단일 구성 요소입니다.
 */

/**
 * 코딩 에이전트를 위한 메인 시스템 프롬프트를 생성하여 반환합니다.
 * 이 함수는 에이전트의 페르소나, 핵심 규칙, 상호작용 스타일, 작업 수행 방법론,
 * 그리고 각 도구의 상세 사용 지침 등 모든 가이드라인을 단일 문자열로 통합합니다.
 * 이 프롬프트의 내용은 모델의 응답 생성 방식에 직접적인 영향을 미칩니다.
 * @returns {string} 에이전트의 행동을 안내하는 완성된 시스템 프롬프트 문자열.
 */
