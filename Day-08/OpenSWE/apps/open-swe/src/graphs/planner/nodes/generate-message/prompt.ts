/**
 * @file Planner 컨텍스트 수집 프롬프트 템플릿
 * @description
 * Planner LLM이 컨텍스트 수집 시 사용하는 프롬프트 템플릿들을 정의합니다.
 *
 * 주요 템플릿:
 * - SYSTEM_PROMPT: 컨텍스트 수집 메인 프롬프트 (가이드라인 포함)
 * - EXTERNAL_FRAMEWORK_DOCUMENTATION_PROMPT: LangGraph 문서 접근 가이드
 * - EXTERNAL_FRAMEWORK_PLAN_PROMPT: LangGraph 특화 계획 가이드
 * - DEV_SERVER_PROMPT: 개발 서버 도구 사용 가이드
 *
 * 프롬프트 특징:
 * 1. 읽기 전용 강조: 파일 수정 금지
 * 2. 효율적인 도구 사용: grep 도구, 병렬 도구 호출
 * 3. 최소 컨텍스트: 계획 생성에 필요한 최소한만 수집
 * 4. LangGraph 특화: LangGraph 관련 작업 시 문서 참조
 */
 ...
 *
 * @description
 * LLM이 컨텍스트 수집 시 사용하는 메인 프롬프트입니다.
 * 읽기 전용 작업만 수행하도록 지시하며, 효율적인 도구 사용 가이드를 제공합니다.
 *
 * 템플릿 변수:
 * - {FOLLOWUP_MESSAGE_PROMPT}: 후속 요청 여부 안내
 * - {EXTERNAL_FRAMEWORK_DOCUMENTATION_PROMPT}: LangGraph 문서 접근 가이드
 * - {EXTERNAL_FRAMEWORK_PLAN_PROMPT}: LangGraph 특화 계획 가이드
 * - {DEV_SERVER_PROMPT}: 개발 서버 도구 가이드
 ...
 *
 * @description
 * LangGraph 관련 작업 시 langgraph-docs-mcp 도구를 사용하여
 * 최신 문서를 참조하도록 안내합니다.
 *
 * 주요 가이드:
 * - list_doc_sources 도구로 문서 목록 조회
 * - fetch_docs 도구로 문서 내용 가져오기
 * - 상대 경로 올바르게 해석
 */
 ...
 *
 * @description
 * LangGraph 관련 작업 시 적용되는 특화된 계획 가이드라인입니다.
 *
 * 주요 가이드:
 * 1. 구조 요구사항: 기존 파일 구조 유지, agent.py는 빈 프로젝트에서만 생성
 * 2. 배포 우선 계획: 미리 빌드된 컴포넌트 사용 (create_react_agent 등)
 * 3. 에러 방지: 상태 업데이트 딕셔너리 반환, 메시지 .content 추출 등
 * 4. 필수 테스팅: dev_server 도구로 런타임 검증
 * 5. 프레임워크 통합: Streamlit, FastAPI 등과의 통합 가이드
 * 6. 중요 문서: LangGraph 핵심 개념, 패턴, 스트리밍 문서 링크
 */
 ...
 *
 * @description
 * dev_server 도구를 사용하여 런타임 동작을 검증하도록 안내합니다.
 *
 * 사용 시점:
 * - LangGraph 에이전트 생성/수정 시
 * - API 엔드포인트 수정 시
 * - 서버 시작 문제 조사 시
 *
 * 지원 기술별 명령어:
 * - LangGraph: langgraph dev
 * - Node.js/React: npm start, yarn dev
